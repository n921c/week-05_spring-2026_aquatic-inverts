...
aq_ins_clean <- aq_ins_clean |> 
  ...
  # log transform the total abundance per lit
  mutate(log_total_avg_per_lit = log(total_avg_per_lit + 1)) |> 

  # ungroup
  ungroup() |> 
  
  # keep only date, taxon, med_sal, and total avg/L
  select(date, taxon, med_sal, log_total_avg_per_lit) |> 

  # re-order taxon levels by max abundance
  mutate(taxon = fct_reorder(taxon, log_total_avg_per_lit, max, .desc = TRUE)) 
...
```{r}
#| label: abundance and salinity plot
#| fig-height: 8
#| fig-width: 8

#  base layer: create an object that stores the plot
abund_salinity_plot <- ggplot(data = aq_ins_clean,
                        # x = salinity
                        # y = log + 1 transformed average abundance/L
                        mapping = aes(x = med_sal,
                                      y = log_total_avg_per_lit,
                                      color = taxon)) +
...
Across all sites, most of the data points are clustered in between a salinity of 0-10 ppt. Additionally, at all sites, there is a small cluster of data points around 40 ppt and 65 ppt, but not much in between 10-40 ppt and 50-60 ppt. For all taxa except for Annelida Polychaete and Annelida Oligochaete, the highest abundance/L points lie in between this salinity range of 0-10 ppt. Ostracod, Copepod, and Cladocera have the most data points that range across average abundances per liter. Thus, it is reasonable to conclude that the lower salinity contributes to higher taxa abundance/L.
